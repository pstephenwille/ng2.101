define('offersResponseModel', [
    'roomsAndRatesData', 'jquery', 'backbone', 'infositeData', 'ajaxTaapModel', 'underscore', 'experiments', 'errorModel', 'countdownModel', 'hotelInfo'
], function(roomsAndRatesData, $, Backbone, infositeData, ajaxTaapModel, _, experiments, ErrorModel, CountdownModel, hotelInfo) {
    "use strict";
    ...
    return Backbone.Model.extend({
        initialize: function(attributes, options) {
            ...
            this.loyaltyData = new Backbone.Model();
            this.leadOffer = new Backbone.Model();
            this.errors = new ErrorModel();
            this.initBedding();
            this.listenTo(options.urlModel, 'sync', _.partial(this.fetch, options));
            this.on('error', this.setDefaultErrorMessage);
            this.hasSentMCTC = false;
        },
        initBedding: function() {
            //init bedding
            var self = this;
            $.each(roomsAndRatesData.rooms, function(index, room) {
                try {
                    // Merge the bed type options into a single string.
                    // TODO: Localize the word "or".
                    if (typeof room.beddingOptions !== 'undefined') {
                        room.beddingOptions = room.beddingOptions.join(' or ');
                    }
                    // Extra bedding
                    if (typeof room.amenities !== 'undefined') {
                        room.extraBeddingOptions = room.amenities['4307'] || room.amenities['4949'] || room.amenities['4948'] || room.amenities['4950'] || room.amenities['4951'] || room.amenities['2044'] || room.amenities['2045'];
                        room.cribBedding = room.amenities['4949'] || room.amenities['4948'] || room.amenities['4950'] || room.amenities['4951'] || room.amenities['2045'];
                        room.rollawayBedding = room.amenities['2044'] || room.amenities['4307'];
                        room.cribBeddingAndRollawayBedding = room.cribBedding && room.rollawayBedding;
                    }
                } catch (e) {
                    self.setDefaultErrorMessage();
                    require('logger', function(logger) {
                        logger.logError('ajaxRoomsAndRates', e);
                    });
                }
            });
        },
        setDefaultErrorMessage: function() {
        ...
        url: function () {
            // _.omit removes undefined parameters from the final querystring
            var additionalParams = _.omit({
                token: infositeData.token,
                brandId: infositeData.hotelBrandId,
                isVip: infositeData.isVIPHotel,
                chid: infositeData.cacheID,
                mctc: this.generateMCTCParam()
            }, function (value) {
                return value === undefined; //if the value is undefined don't include
            });
            return '/api/infosite/' + infositeData.hotelId + '/getOffers?'
                + $.param(additionalParams, true) + '&'
                + this.urlModel.serializeToQuery();
        },
        // This method will only generate the MCTC parameter once.
        generateMCTCParam: function () {
            if (!this.hasSentMCTC) {
                this.hasSentMCTC = true;
                return infositeData.marketChannel === -1 ? undefined : infositeData.marketChannel;
            }
        },
        // Process ETP offers
        ...
        //TODO: Push to Java
        fixPrices: function(price) {
            if (price) {
                var crossOutPrice = price.discountAmount ? price.discountAmount + price.price : void 0;
                var totalPrice = parseFloat(price.unformattedTotalPrice);
                price.crossOutPrice = _.isNumber(crossOutPrice) && !isNaN(crossOutPrice) ? crossOutPrice : void 0;
                price.totalPrice = _.isNumber(totalPrice) && !isNaN(totalPrice) ? totalPrice : void 0;
            }
        },
        ...
        adaptOffers: function(response, ts) {
            ...
            var modelData = {
                ...
                rooms: [],
                //TODO: Do the following attributes really belong to this model?
                displayExpediaRewardsColumn: roomsAndRatesData.displayExpediaRewardsColumn,
                descripColspan: roomsAndRatesData.displayExpediaRewardsColumn ? 5 : 4,
                isVipAccess: roomsAndRatesData.isVipAccess,
                displayTaapContent: ajaxTaapModel.displayTaapContent,
                taapCommission: {
                    esr: ajaxTaapModel.taapCommissions.ESR,
                    eem: ajaxTaapModel.taapCommissions.EEM,
                    gds: ajaxTaapModel.taapCommissions.GDS
                },
                showTaapAirAttach: (experiments[9339] === 1 || experiments[10202] === 1)
            };
            ...
            $.each(response.offers || [], function(index, offer) {
                ...
                var roomData = $.grep(modelData.rooms, function(room) {
                ...
                if (roomData.length === 0) {
                    ...
                    roomData = $.grep(roomsAndRatesData.rooms, function(room) {
                        // The room type codes now have the same datatypes on the server side.
                        return room.roomTypeCode === offer.roomTypeCode;
                    });
                ...
                if (typeof offer.amenities !== 'undefined' && typeof roomData.amenities !== 'undefined') {
                    ...
                    offer.displayNewAmenities = infositeData.isFrenchPOSa || experiments[10532] === 1;
                ...
                offer.isPwPRewardsMsg = experiments[7936] === 1 && experiments[8623] === 1;
                ...
                offer.showTonightOnlyBadge = offer.tonightOnlyDRRPresent && experiments[9366] === 1;
            ...
            self.adaptEtp(modelData, hotelInfo.suppressETP);
            self.sortAndIndex(modelData);
            return modelData;
        },
        getLeadOffer: function(adaptedOffers) {
            try {
                var leadOffer = adaptedOffers.rooms[0].offers[0];
                if(!leadOffer.exceedsMaxChildren && !leadOffer.exceedsMaxGuests && leadOffer.nightlyRates){
                    return leadOffer;
                }
            } catch (err) {}
        },
        parse: function(response) {
            ...
            var adaptedOffers = this.adaptOffers(response, ts);
            this.loyaltyData.set(response.loyaltyData);
            this.errors.set({
                availabilityErrors: response.availabilityErrors,
                validationErrors: response.validationErrors,
                ts: ts
            });
            if (response.airAttach) {
                var attrs = _.extend(response.airAttach, {id: 'AirAttach'});
                this.banners.add(new CountdownModel(attrs), {merge: true});
            } else {
                this.banners.remove('AirAttach');
            }

            var leadOffer = this.getLeadOffer(adaptedOffers);
            if (leadOffer) {
                this.leadOffer.set(leadOffer);
            } else {
                this.leadOffer.clear();
            }

            return adaptedOffers;
        },

        // Sort and assign indexes to Rooms and Offers
